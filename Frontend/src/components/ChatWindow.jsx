...
const ChatWindow = ({ chat, messages, sendMessage, currentUser, openSidebar, onLongPress }) => {
  ...
  return (
    <div className="flex-grow flex flex-col bg-n-8 text-n-1">
      <div className="bg-n-7 text-n-0 p-4 flex items-center">
        <button
          onClick={openSidebar}
          className="md:hidden mr-4 text-n-0"
        >
          ☰
        </button>
      ...
      <div className="flex-grow overflow-y-auto p-4">
        {messages.map((message) => (
          ...
          >
            <div
              className={`inline-block max-w-xs md:max-w-md rounded-lg px-4 py-2 ${
                message.sender === currentUser.uid ? 'bg-n-6 text-n-0' : 'bg-n-1 text-n-7'
              }`}
            >
              {chat.isGroup && (
                <p className="font-semibold text-sm" style={{ fontFamily: 'var(--font-sora)' }}>
                  {message.sender === currentUser.uid ? currentUser.displayName : message.senderDisplayName}
                </p>
              )}
              <p>{message.text}</p>
